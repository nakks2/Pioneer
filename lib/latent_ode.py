...
class ODE(VAE_Baseline):
	def __init__(self, w_node_to_edge_initial,ode_hidden_dim, encoder_z0, decoder_node,decoder_edge, diffeq_solver,
		...
		self.w_node_to_edge_initial = w_node_to_edge_initial #A_ij = f(W([h_i||h_j]))
	...
	def get_reconstruction(self, batch_en,batch_de):
        ...
        #Encoder:
		first_point_mu= self.encoder_z0(batch_en.x, batch_en.edge_weight,
		...
		first_point_std = torch.ones_like(first_point_mu)
        ...
        # Decoder:
		pred_node = self.decoder_node(sol_y[:K_N,:,:],is_edge = False,is_rie=self.Rie)
		pred_edge = self.decoder_edge(sol_y[K_N:, :, :],is_edge = True,is_rie=False)
		assert (not torch.isnan(pred_edge).any())
		assert (not torch.isnan(pred_node).any())
		...
		return pred_node,pred_edge, all_extra_info
