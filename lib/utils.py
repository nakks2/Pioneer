...
def test_data(model, pred_length, condition_length, dataloader, device, args, kl_coef,ce=False):
	encoder, decoder, graph, num_batch = dataloader.load_test_data(pred_length=pred_length,
																   condition_length=condition_length,ce=ce)
	...
	with torch.no_grad():
		for _ in tqdm(range(num_batch)):
			...
			results = model.compute_all_losses(batch_dict_encoder, batch_dict_decoder, batch_dict_graph, args.num_atoms,
											   edge_lamda=args.edge_lamda, kl_coef=kl_coef, istest=True,ce=ce,dataset=args.dataset)
	...
	total["RMSE"] = numpy.min(RMSE_each)
	if dataloader.dataset == "CReSIS":
		total["MAPE"] = numpy.min(MAPE_each)
	return total