...
from lib.create_ricci_ode_model import create_ODE_model
import lib.load_data
from lib.utils import test_data

parser = argparse.ArgumentParser('Pioneer')
...
parser.add_argument('--pred_length', type=int, default=10, help="times in ODE ")
parser.add_argument('--condition_length', type=int, default=20, help="times in encoder. ")
...
parser.add_argument('--edge_lamda', type=float, default=0.7, help='edge weight')
...
parser.add_argument('--alpha', type=float, default=6e-1, help='is_edge')
parser.add_argument('--beta', type=float, default=1e-2,help="balance of ricci flow")
parser.add_argument('--augment_dim', type=int, default=0, help='augmented dimension')
parser.add_argument('--Riemannan', type=bool, default=True, help='Riemannan')
...
if args.dataset == "social":
    ...
    args.output_dim = 2
elif args.dataset == "weather" or args.dataset == "CReSIS":
...
if __name__ == '__main__':
    ...
    train_encoder, train_decoder, train_graph_ex, train_graph, train_batch, num_atoms , num_features= dataloader.load_train_data(is_train=True)
    val_encoder, val_decoder,val_graph_ex, val_graph, val_batch, _, _ = dataloader.load_train_data(is_train=False)
    args.num_atoms = num_atoms
    input_dim = num_features
    args.feature_max=dataloader.features_max
    args.feature_min=dataloader.features_min

    ############ Model SetUp
    # Create the model

    model = create_ODE_model(args, input_dim, device)
    ...
    def train_single_batch(model,batch_dict_encoder,batch_dict_decoder,batch_dict_graph,kl_coef):
        ...
        optimizer.zero_grad()
        train_res = model.compute_all_losses(batch_dict_encoder, batch_dict_decoder, batch_dict_graph,args.num_atoms,edge_lamda = args.edge_lamda, kl_coef=kl_coef,istest=False,dataset=args.dataset)
        ...
        # train_res, loss
        return loss_value,train_res["MAPE"],train_res['MSE']
    ...
    def train_epoch(epo):
        ...
        for itr in tqdm(range(train_batch)):
            ...
            loss, MAPE, MSE, = train_single_batch(model,batch_dict_encoder,batch_dict_decoder,batch_dict_graph,kl_coef)



            #saving results
            loss_list.append(loss), MAPE_list.append(MAPE), MSE_list.append(MSE)
        ...
        message_train = 'Epoch {:04d} [Train seq (cond on sampled tp)] | Loss {:.6f} | MAPE {:.6F} | RMSE {:.6F} |'.format(
            epo,
            np.mean(loss_list), np.mean(MAPE_list),np.sqrt(np.mean(MSE_list)))
    ...
    def val_epoch(epo, kl_coef):
        ...
        for itr in tqdm(range(val_batch)):
            ...
            val_res = model.compute_all_losses(batch_dict_encoder, batch_dict_decoder, batch_dict_graph,
                                               ...
                                               istest=False,dataset=args.dataset)
    ...
    if args.load is not None:
        test_res= test_data(model, args.pred_length, args.condition_length, dataloader,
                                                   device=device, args=args, kl_coef=0,ce=True)

        message_test = 'Epoch {:04d} [Test seq (cond on sampled tp)] | Loss {:.6f} | MAPE {:.6F} | RMSE {:.6F} |'.format(
            0,
    ...
    for epo in range(1, args.niters + 1):
        ...
        if epo % n_iters_to_viz == 0:
            ...
            if test_res["RMSE"] < best_test_RMSE:
